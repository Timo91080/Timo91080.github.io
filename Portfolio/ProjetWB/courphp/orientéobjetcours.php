...
//INTERFACE
interface Travailleur { //L'interface définit simplement des méthode que l'on veut rendre obligatoire sur tout les objet qui se définisse comme des travailleurs en gros faut que les class est les méthode que le contrat du travailleurs demande//
...
class Employe implements Travailleur { //en gros le implements veut dire que la class employé a signé le contrat du travailleur
 private $nom;
 private $prenom;
 protected $age;// quand on met en private ou en protected sa veut dire quon peut utiliser bah la variable $age  que dans les fonction de la class Employé et protected dans les class qui hérite//
...
public function setAge($age) { //Cette fonction a pour but de recevoir donc la c'est recevoir un age et de ce dire ok bah mon age c'est l'age quon ma donné envoyé comme vous voulez//
...
class Etudiant implements Travailleur 
 ...
 function faireTravailler(Travailleur $objet) // la fonction va prendre n'importe quel objet qui impléments la fonctionc Travailleur c'est a dire n'importe quel objet qui est dans une class qui a signé le contrat du travailleurs //
 {
